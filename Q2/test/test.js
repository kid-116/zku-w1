...
describe("HelloWorld", function () {
    ...
    it("Should return true for correct proof", async function () {
        ...
        // groth16.fullProve is an async function which returns the proof and public signals
        // given INPUT signals, circuit path and zero-knowledge key file for the circuit which is
        // generated during the Powers Of Tau ceremony
        ...
        // To verify if the output signal is correct
        console.log('2x3 =', publicSignals[0]);
        
        // Extracts the data required by the Solidity verifer contract as a single string
        const calldata = await groth16.exportSolidityCallData(proof, publicSignals);
    
        // Split the calldata string into the numerous arguements it holds separated by commas
        const argv = calldata.replace(/["[\]\s]/g, "").split(',').map(x => BigInt(x).toString());
    
        // Initialize the final parameters required for the call to the verifier contract
        const a = [argv[0], argv[1]];
        const b = [[argv[2], argv[3]], [argv[4], argv[5]]];
        const c = [argv[6], argv[7]];
        const Input = argv.slice(8);

        // Expect the result of the verification to be true as it is a correct proof
...
describe("Multiplier3 with Groth16", function () {
    ...
    it("Should return true for correct proof", async function () {
        ...
        console.log('2x3x9 =', publicSignals[0]);
